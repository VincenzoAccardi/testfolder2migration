...
#Region "Documentation"
    ' ********** ********** ********** **********
    ' E F T - Ingenico protocol implementation
    ' ---------- ---------- ---------- ----------
...
#Region "Global member variables"
    ...
    Protected WithEvents TheIngenicoEft As Ingenico = New Ingenico
...
#End Region
    ...
    Public Overridable Function EFTHandler(ByRef taobj As TA, ByRef MyTaMediaRec As TaMediaRec, ByVal IsVoid As Boolean, Optional ByVal AddEftInfoToCurrentTa As Boolean = True, Optional ByVal CreateAndPrintAnEftTa As Boolean = True) As Short
        ...
        Try
            ...
            ' call the worker
            If TheIngenicoEft.PayIngenico(TheAmount, Me.theModCntr, IsVoid) = 0 Then
                Do While TheIngenicoEft.DialogActiv = True
                    Sleep(100)
            ...
            LOG_Error(getLocationString("EFTHandler"), "End payment thread (ESITO = <" & TheIngenicoEft.TransactionResult & _
                                                        "> STATO = <" & TheIngenicoEft.State.ToString & ")")

            If String.Equals(TheIngenicoEft.TransactionResult, "00") Then
                ' Esito reports OK
                Dim TxSaved As Boolean = EFTResultHandler(taobj, MyTaMediaRec, TheIngenicoEft.GetTicket(), AddEftInfoToCurrentTa, CreateAndPrintAnEftTa)
                If Not IsVoid Then
                    SwapElectronicMedia(taobj, MyTaMediaRec, TheIngenicoEft.PaymentResult.ACQUIRER_ID)
                    SetLastPaymentTransactionAmount(MyTaMediaRec.dTaPaidTotal * (10 ^ m_theModCntr.iEXACTNESS_IN_DIGITS))
                    SetLastPaymentTransactionNumber(TheIngenicoEft.PaymentResult.STAN)
                End If
                If TxSaved Then
                    EFTHandler = 0
                End If
            Else
                ' Esito reports BAD
                TPMsgBox(PosDef.TARMessageTypes.TPERROR, TheIngenicoEft.ErrorMessage, 0, theModCntr, TheIngenicoEft.ErrorMessage)
                LOG_Error(getLocationString("EFTHandler"), TheIngenicoEft.ErrorMessage)
            End If

            If String.Equals(TheIngenicoEft.TransactionResult, "  ") Then
                ' we don't know what is really happened
                If ASK_FOR_MANUAL_PAYMENT Then
                    EFTHandler = AskForManualPayment()
                End If
            End If

            ' abort the thread if it is alive
            TheIngenicoEft.AbortPayment()
    ...
    Public Overridable Function EFTClosureHandler(ByRef taobj As TA, ByRef MyTaMediaRec As TaMediaRec, ByVal IsVoid As Boolean, Optional ByVal AddEftInfoToCurrentTa As Boolean = True, Optional ByVal CreateAndPrintAnEftTa As Boolean = True) As Short
        ...
        Try
            ...
            ' call the worker
            If TheIngenicoEft.CloseIngenico(Me.theModCntr) = 0 Then
                Do While TheIngenicoEft.DialogActiv = True
                    Sleep(100)
                    System.Windows.Forms.Application.DoEvents()
                Loop
            ...
            szTicket = TheIngenicoEft.GetTicket()
            LOG_Error(getLocationString("EFTClosureHandler"), "End closure thread (TextLen = <" & szTicket.Length & _
                                                        "> STATO = <" & TheIngenicoEft.State.ToString & ")")

            If TheIngenicoEft.State = Ingenico.OperationStates.Ingenico_SUCCESS AndAlso Not String.IsNullOrEmpty(szTicket) Then
                ' Esito reports OK
                Dim TxSaved As Boolean = EFTResultHandler(taobj, MyTaMediaRec, TheIngenicoEft.GetTicket(), AddEftInfoToCurrentTa, CreateAndPrintAnEftTa, True)
                EFTClosureHandler = 0
            Else
                ' Esito reports BAD
                TPMsgBox(PosDef.TARMessageTypes.TPERROR, TheIngenicoEft.ErrorMessage, 0, theModCntr, TheIngenicoEft.ErrorMessage)
                LOG_Error(getLocationString("EFTClosureHandler"), TheIngenicoEft.ErrorMessage)
            End If
    ...
    Public Function EFTResultHandler(ByRef taobj As TA, ByRef MyTaMediaRec As TaMediaRec, ByRef ReceiptText As String, Optional ByVal AddEftInfoToCurrentTa As Boolean = True, Optional ByVal CreateAndPrintAnEftTa As Boolean = True, Optional ByVal IsClosure As Boolean = False) As Boolean
        ...
        Try
            ...
            If Not m_ReceiptText.Length > 0 AndAlso Not IsClosure Then
                ...
                'm_ReceiptText += "Nome Acquirer: " & TheIngenicoEft.DatiAutorizzazioneECarta.NOME_ACQUIRER & vbCrLf
                m_ReceiptText += "Cod. Acquirer: " & TheIngenicoEft.PaymentResult.ACQUIRER_ID & vbCrLf
                m_ReceiptText += "PAN: " & TheIngenicoEft.AdditionaPaymentVoidOKResult.PAN & vbCrLf
                'm_ReceiptText += "Scadenza: " & TheIngenicoEft.DatiAutorizzazioneECarta.SCADENZA_CARTA_MMAA & vbCrLf
                m_ReceiptText += "N.Op: " & TheIngenicoEft.PaymentResult.ONLINE_OPERATION_COUNTER & vbCrLf
                m_ReceiptText += "Cod. Aut.: " & TheIngenicoEft.AdditionaPaymentVoidOKResult.AUTHORIZATION_CODE & vbCrLf
                m_ReceiptText += "EURO " & Format(TheAmount, m_theModCntr.getFormatString4Price()) & vbCrLf
                If TheIngenicoEft.PaymentResult.CARD_TYPE = "4" Then
...
#Region "Italy"
    ...
    Public Sub IngenicoStatusChanged(ByVal state As Integer, ByVal message As String) Handles TheIngenicoEft.IngenicoStatusChanged

        If state = Ingenico.OperationStates.Ingenico_ERROR Then
            WriteInfo(String.Format("{0}", message))

        ElseIf state = Ingenico.OperationStates.Ingenico_IN_PROGRESS Then
            WriteInfo(String.Format("{0}", message))

        ElseIf state = Ingenico.OperationStates.Ingenico_SUCCESS Then
            WriteInfo(String.Format("{0}", message))
        End If
    ...
    Public Sub IngenicoRemainindSecondsChanged(ByVal s As Integer) Handles TheIngenicoEft.IngenicoRemainingSecond
    ...
    Public Sub CancelOperationIsAvailable(ByVal available As Boolean)
        Try
            ...
            If myForm.cmdCancel.InvokeRequired Then
                myForm.cmdCancel.Invoke(New IngenicoCancelOperationIsAvailableChangedEventHandler(AddressOf IngenicoCancelOperationIsAvailableChanged), available)
            Else
    ...
    Private Delegate Sub IngenicoCancelOperationIsAvailableChangedEventHandler(ByVal available As Boolean)
    Public Sub IngenicoCancelOperationIsAvailableChanged(ByVal available As Boolean) Handles TheIngenicoEft.IngenicoCancelOperationIsAvailableChanged
    ...
    Public Sub EFTDeviceMessage(ByRef szMessage As String) Handles myForm.EFTDeviceMessage
        ...
        If szMessage = "Abort" Then
            ...
            TheIngenicoEft.AbortPayment(msg)
        End If
        ...
        Try
            ...
            Select Case szTheRequest
                ...
                Case "GetLastPaymentTransactionAmount"
                    DoServiceRequest = GetLastPaymentTransactionAmount()
                    Exit Select

                Case "GetLastPaymentTransactionNumber"
                    DoServiceRequest = GetLastPaymentTransactionNumber()
                    Exit Select
