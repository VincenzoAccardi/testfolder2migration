...
Public Class BPEController
    ...
    Private Const TYPE_SPECIFIQUE As String = "BPE"                                                         ' <-- Costante per identificare il sottotipo nell'applicazione (Elettronici carta su pos hardware)

    ' Modalità del servizio Proxy di Argentea
    Private Const MODE_PROXY_USE As ClsProxyArgentea.enTypeProxy = ClsProxyArgentea.enTypeProxy.Pos         ' <-- Il Proxy servizio avviato in modalità Hardware
    ...
    Public Function Dematerialize(ByRef Parameters As Dictionary(Of String, Object)) As IBPReturnCode Implements IBPDematerialize.Dematerialize
        ...
        Try
            ...
            If Not Common.ApplyFilterStyleSheet(m_TheModcntr, m_taobj, "BPCType.xslt", sTotalTransaction) Then
                ...
                m_LastStatus = OPR_ERROR_FILE_FILTER
            ...
            If TotalTransaction = 0 Then
                ...
                m_LastStatus = NOT_ERROR_CALL_DEMAT_INCONGRUENT
            ...
            If m_CurrMedia.dTaPaidTotal > 0 Then
                ...
                If m_PayableAmout <= 0 Then
                    ...
                    ' Valorizzo l'errore nei Params per uscire dall
                    ' iterazione del modulo corrente
                    m_LastStatus = NOT_ERROR_EXCEDED_PAYABLE
                    m_LastErrorMessage = "Il pagamento eccede sul totale della vendita in corso!"
            ...
            Else
                ...
                ' Valorizzo l'errore nei Params per uscire dall
                ' iterazione del modulo corrente
                m_LastStatus = NOT_ERROR_ALREADY_PAYED
                m_LastErrorMessage = "Il pagamento ricopre già l'intero totale della vendita in corso!"
    ...
    Public Function Void(ByRef Parameters As Dictionary(Of String, Object)) As IBPReturnCode Implements IBPDematerialize.Void
        ...
        Try
            ...
            If m_CurrMedia Is Nothing _
                ...
                ' Etichettiamo l'errore per la gestione
                m_LastStatus = OPR_ERROR_MEDIA_TYPE_NOT_VALID
            ...
            If TotalToVoid >= 0 Then
                ...
                ' Valorizzo l'errore nei Params per uscire dall
                ' iterazione del modulo corrente
                m_LastStatus = NOT_ERROR_CALL_VOID_INCONGRUENT
            ...
            Else
                ...
                If CBool(m_CurrMedia.GetPropertybyName("bGroupedPaymentsOrder")) = False Then
                    ...
                    If HandleSingleVoidBPCall(False, CurrentBarcode, CurrentCrcTransactionId, CurrentTotValueBp) Then
    ...
    Private Function _initializeParameters(ByRef funcName As String, ByRef parameters As Dictionary(Of String, Object), showOperatorError As Boolean) As IBPReturnCode
        ...
        Try
            ...
            If m_TheModcntr Is Nothing Or m_taobj Is Nothing Then
                ...
                ' Etichettiamo l'errore per la gestione
                m_LastStatus = OPR_ERROR_START_PROXY_INTERNAL
